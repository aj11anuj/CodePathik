...
async def fetch_file_content(repo_url: str, file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        repo_info = requests.get(f"https://api.github.com/repos/{user}/{repo}", headers=headers)
...
async def process_github_repo(url: str) -> Dict[str, Union[str, int, Dict]]:
    ...
    if (meta_res := requests.get(repo_api, headers=headers)).status_code != 200:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        try:
            ...
            if (r := requests.get(zip_url, headers=headers)).status_code != 200:
